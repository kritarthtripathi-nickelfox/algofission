...
export default function OurServices({ data }) {
  return (
    <section className="container mx-auto flex flex-col items-center mt-20">
      <Fade bottom>
        <h1 className="text-5xl text-theme-blue text-center font-bold">Solution that Changes Lives</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 sm:gap-2 xl:gap-8 justify-items-center sm:ml-5">
        {
          data.map((item, index) => (
            <Fade bottom delay={500 * index}>
              <Button type="link" href={`/service/${item.id}`}>
                <div className="group rounded-2xl shadow-xl w-auto w-11/12 m-3 transform transition duration-500 sm:hover:scale-70 xl:hover:scale-110 portofolio-card">
                  ...
                  <div className="py-4">
                    ...
                    <p className="font-light text-gray-400 text-center mr-3 ml-3" style={{ textAlign: "justify" }}>{item.type}</p>
                  </div>
                </div>
              </Button>
            </Fade>
